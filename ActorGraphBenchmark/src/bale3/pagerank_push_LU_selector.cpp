...
// 
 *****************************************************************/ 
/*! \file pagerank_push_LU_selector.cpp
...
class PageRankSelector: public hclib::Selector<1, PageRankPkt> {
public:
...
private:
    ...
    void req_process(PageRankPkt pkg, int sender_rank) {
        // add recieved pagerank factors from incoming neighbors to local pagerank factor
        pg_rnk_cur_itr_[pkg.dest_idx] += pkg.prf_y;
    }
};
...
double calculate_curr_iter_pagerank(sparsemat_t* L, double* page_rank, double* page_rank_curr_iter, int64_t* degrees_total) {
    ...
    hclib::finish([=]() {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            // local pagerank factor
            prPKG.prf_y = page_rank[y] / (double) degrees_total[y];
            // push local pagerank factor to outgoing neighbors
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                prPKG.dest_idx = L->lnonzero[k] / THREADS;
