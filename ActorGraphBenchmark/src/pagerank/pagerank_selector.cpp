...
public:
    DegreeSelector(int64_t* deg_ttl) : deg_ttl_(deg_ttl) {
...
public:
    PageRankSelector(double* pg_rnk, double* pg_rnk_cur_itr, int64_t* deg_ttl) : pg_rnk_(pg_rnk), pg_rnk_cur_itr_(pg_rnk_cur_itr), deg_ttl_(deg_ttl) {
...
private:
    ...
    void req_process(PageRankPkt pkg, int sender_rank) {
        pg_rnk_cur_itr_[pkg.src_idx] += pkg.pr_y / (double)pkg.deg_y;
    }

    void resp_process(PageRankPkt pkg, int sender_rank) {
        PageRankPkt pkg2;
        pkg2.pr_y = pg_rnk_[pkg.src_idx];
        pkg2.deg_y = (double)deg_ttl_[pkg.src_idx];
        pkg2.src_idx = pkg.dest_idx;
        pkg2.pe_dest = pkg.pe_dest;
        send(REQUEST, pkg2, pkg2.pe_dest);
    }
};

double calculate_degrees(sparsemat_t* L, int64_t* degrees_total) {

    DegreeSelector* degreeSelector = new DegreeSelector(degrees_total);
    ...
    hclib::finish([=]() {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                ...
                degrees_total[y]++;
                degreeSelector->send(REQUEST, prPKG, pe);
            }
        }
...
double calculate_curr_iter_pagerank(sparsemat_t* L, double* page_rank, double* page_rank_curr_iter, int64_t* degrees_total) {
    ...
    PageRankSelector* pagerankSelector = new PageRankSelector(page_rank, page_rank_curr_iter, degrees_total);
    ...
    hclib::finish([=]() {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                ...
                prPKG.dest_idx = y;
                pagerankSelector->send(RESPONSE, prPKG, prPKG.pe_src);

                prPKG.pr_y = page_rank[y];
                prPKG.deg_y = (double)degrees_total[y];
                pagerankSelector->send(REQUEST, prPKG, prPKG.pe_src);
            }
        }

        pagerankSelector->done(RESPONSE);
    });
...
int main(int argc, char* argv[]) {
    ...
    hclib::launch(deps, 2, [=] {
        ...
        laptime_pagerank = pagerank_selector(L, page_rank, page_rank_curr_iter, degrees_total);
        lgp_barrier();
