 ...
 *      NOTE: this is the push version of pagerank
 */
#include <math.h>
#include <shmem.h>
extern "C" {
...
double pagerank_agi(sparsemat_t* L, double* lpage_rank, double* page_rank_curr_iter) {//, int64_t* degrees_total) {
    ...
    while (true) {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            int64_t deg_y_upper = L->loffset[y+1];
            int64_t deg_y_lower = L->loffset[y];
            double prf_y = lpage_rank[y] / (double) (deg_y_upper - deg_y_lower);

            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                int64_t pe_dest = L->lnonzero[k] % THREADS;
                int64_t dest_idx = L->lnonzero[k] / THREADS;

                static long lock = 0;
                shmem_set_lock(&lock);

                double prf_dest = shmem_double_g(page_rank_curr_iter + dest_idx, pe_dest);
                prf_dest += prf_y;
                shmem_p(page_rank_curr_iter + dest_idx, prf_dest, pe_dest);
...
*/
    ...
    double* page_rank_curr_iter = (double*) lgp_all_alloc(pr_size, sizeof(double));//pr_size, sizeof(double));
