...
static int64_t calculate_degrees_push_process(sparsemat_t* L, int64_t* degrees_total, convey_t* conv, int64_t done) {
    DegreePkt pkg;
...
double calculate_degrees(sparsemat_t* L, int64_t* degrees_total) {
    ...
    if (convey_begin(conv, sizeof(DegreePkt)) != convey_OK) return(-1);
    ...
    for (int64_t y = 0; y < L->lnumrows; y++) {
        for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
            ...
            prPKG.i = L->lnonzero[k] / THREADS;
            degrees_total[y]++;
            calculate_degrees_push_process(L, degrees_total, conv, 0);

            if (!convey_push(conv, &prPKG, pe)) {
                break;
            }
        }
    }
    while (calculate_degrees_push_process(L, degrees_total, conv,  1)) {}// keep popping til all threads are done
...
static int64_t calculate_curr_iter_pagerank_push_process(sparsemat_t* L, double* page_rank, double* page_rank_curr_iter, int64_t* degrees_total, convey_t* conv, int64_t done) {
    PageRankPkt pkg;
    
    while (convey_pull(conv, &pkg, NULL) == convey_OK) {
        page_rank_curr_iter[pkg.src_idx] += pkg.pr_y / (double)pkg.deg_y;
    }
...
double calculate_curr_iter_pagerank(sparsemat_t* L, double* page_rank, double* page_rank_curr_iter, int64_t* degrees_total) {
    ...
    convey_begin(conv, sizeof(PageRankPkt));
    convey_begin(requests, sizeof(PageRankPkt));
    convey_begin(replies, sizeof(PageRankPkt));
    ...
    while (more = convey_advance(requests, (y == L->lnumrows)),
        ...
        for (; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                prPKG.pe_src = L->lnonzero[k] % THREADS;
                prPKG.src_idx = L->lnonzero[k] / THREADS;
                prPKG.pe_dest = MYTHREAD;
                prPKG.dest_idx = y;
                if (! convey_push(requests, &prPKG, prPKG.pe_src)) break;

                prPKG.pr_y = page_rank[y];
                prPKG.deg_y = (double)degrees_total[y];
                calculate_curr_iter_pagerank_push_process(L, page_rank, page_rank_curr_iter, degrees_total, conv, 0);
                if (!convey_push(conv, &prPKG, prPKG.pe_src)) break;
            }
        }

        while (calculate_curr_iter_pagerank_push_process(L, page_rank, page_rank_curr_iter, degrees_total, conv, 1)) {}// keep popping til all threads are done
        ...
        while (convey_pull(requests, ptr, &from) == convey_OK) {
            ...
            prPKG.src_idx = ptr->dest_idx;
            prPKG.pe_dest = ptr->pe_dest;
        ...
        while (convey_pull(replies, ptr, NULL) == convey_OK) {page_rank_curr_iter[ptr->src_idx] += ptr->pr_y / (double)ptr->deg_y;}
    }
...
int main(int argc, char* argv[]) {
    ...
    laptime_pagerank = pagerank_conveyor(L, page_rank, page_rank_curr_iter, degrees_total);
    lgp_barrier();
