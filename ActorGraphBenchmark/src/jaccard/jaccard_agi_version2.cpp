...
double jaccard_agi(sparsemat_t* A2, double* jaccard_index) {
    // Start timing
    ...
    for (int64_t v = 0; v < A2->lnumrows; v++) {             //vertex v (local)
        ...
        for (int64_t k = A2->loffset[v]; k < A2->loffset[v+1]; k++) {
            ...
            int64_t pe_u = u % THREADS;
            int64_t index_u = u / THREADS;
            if (vertex_v < u) continue;
            ...
            // calculate union
            int64_t deg_u_upper = lgp_get_int64(A2->offset, u + THREADS);
            int64_t deg_u_lower = lgp_get_int64(A2->offset, u);
            int64_t deg_u = deg_u_upper - deg_u_lower;
            int64_t sum_union = deg_u + deg_v - jaccard_index[pos];

            // calculate jaccard index using intersection and union
            jaccard_index[pos] = jaccard_index[pos] / sum_union;
...
int main(int argc, char* argv[]) {
    ...
    sparsemat_t* A2 = gen_erdos_renyi_graph_dist(10, 0.5, 0, 0, 12345);
    L = A2;
    tril(L, -1);
    ...
    int64_t ljaccard_size = L->lnnz;
    double* jaccard_index = (double*)lgp_all_alloc(ljaccard_size, sizeof(double));
    ...
    laptime_jaccard = jaccard_agi(A2, jaccard_index);
    lgp_barrier();
